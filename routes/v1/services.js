...
router.get('/', async (req, res) => {
  try {
    const db = getDb();
    const services = await db.prepare('SELECT * FROM services WHERE active = 1 ORDER BY sort_order, name').all();
    res.json(services);
  } catch (err) {
...
router.post('/', requireRole('admin'), async (req, res) => {
  try {
    ...
    const result = await db.prepare('INSERT INTO services (name, description, duration, price, color, sort_order) VALUES (?, ?, ?, ?, ?, ?)').run(
...
router.put('/:id', requireRole('admin'), async (req, res) => {
  try {
    ...
    await db.prepare('UPDATE services SET name=?, description=?, duration=?, price=?, color=?, sort_order=? WHERE id=?').run(
