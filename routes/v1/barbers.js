...
router.post('/', requireRole('admin'), (req, res) => {
  try {
    ...
    if (username && password) {
      ...
      const userResult = db.prepare('INSERT INTO users (username, password, role, display_name, phone, email) VALUES (?, ?, ?, ?, ?, ?)').run(
...
// Update barber
router.put('/:id', requireRole('admin', 'barber'), (req, res) => {
  try {
    const db = getDb();
...
// Days off
router.get('/:id/days-off', (req, res) => {
  try {
    const db = getDb();
    const daysOff = db.prepare('SELECT * FROM days_off WHERE barber_id = ? ORDER BY date').all(req.params.id);
    res.json(daysOff);
  } catch (err) {
...
router.post('/:id/days-off', requireRole('admin', 'barber'), (req, res) => {
  try {
    const db = getDb();
    const { date, reason } = req.body;
    if (!date) return res.status(400).json({ error: 'נא לציין תאריך' });

    db.prepare('INSERT OR REPLACE INTO days_off (barber_id, date, reason) VALUES (?, ?, ?)').run(req.params.id, date, reason || '');
...
router.delete('/:id/days-off/:dayOffId', requireRole('admin', 'barber'), (req, res) => {
  try {
    const db = getDb();
    db.prepare('DELETE FROM days_off WHERE id = ? AND barber_id = ?').run(req.params.dayOffId, req.params.id);
